...
using CommandResult = Result<CartCheckoutResult>;
...
) : IRequestHandler<CartCheckoutCommand, CommandResult>
{
    public async Task<CommandResult> Handle(CartCheckoutCommand command, CancellationToken ct)
    {
        // 1. Load Cart
        ...
        return CommandResult.Success(new CartCheckoutResult(order.Id, order.TotalAmount));
    }
}